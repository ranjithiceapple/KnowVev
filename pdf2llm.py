...
def chunk_content_for_rag(
    ...
    """
    Chunk content blocks for RAG with semantic boundaries and rich structural metadata.
    ...
    Returns:
        ...
        {
            'content': str,
            'metadata': dict (with structural fields),
            'token_count': int,
            'chunk_id': int
        }
    """
    ...
    def extract_chunk_metadata(chunk_blocks):
        """Extract rich metadata from a chunk's blocks"""
        # Basic metadata
        metadata = {
            'chunk_id': chunk_id,
            'pages': list(set([b['page'] for b in chunk_blocks])),
            'types': list(set([b['type'] for b in chunk_blocks])),
            'start_block': chunk_blocks[0]['position'],
            'end_block': chunk_blocks[-1]['position']
        }
        ...
        for b in chunk_blocks:
            block_meta = b.get('metadata', {})

            # Collect sections
            if block_meta.get('section'):
                sections.append(block_meta['section'])

            # Collect headings
            if b['type'] == 'heading':
                headings.append(b['content'])
                if block_meta.get('heading_level'):
                    heading_levels.append(block_meta['heading_level'])

        # Add structural fields
        if sections:
            metadata['sections'] = list(set(sections))
            metadata['primary_section'] = sections[0]  # First section in chunk

        if headings:
            metadata['headings'] = headings
            metadata['primary_heading'] = headings[0]

        if heading_levels:
            metadata['heading_levels'] = list(set(heading_levels))
            metadata['min_heading_level'] = min(heading_levels)

        # Section hierarchy from the first block
        if chunk_blocks[0].get('metadata', {}).get('section_hierarchy'):
            metadata['section_hierarchy'] = chunk_blocks[0]['metadata']['section_hierarchy']

        # Check if chunk contains tables or figures
        metadata['has_table'] = 'table' in metadata['types']
        metadata['has_figure'] = 'figure' in metadata['types']
        metadata['has_heading'] = 'heading' in metadata['types']
    ...
    for i, block in enumerate(content_blocks):
        ...
        if block_type in ['table', 'figure'] and keep_tables_intact:
            # If table/figure is larger than chunk_size, give it its own chunk
            if block_tokens > chunk_size:
                ...
                # Add table/figure as its own chunk
                chunks.append({
                    'content': block_content,
                    'metadata': {
                        **extract_chunk_metadata([block]),
                        'oversized': True  # Flag for content larger than chunk_size
                    },
            ...
            # If adding table/figure would exceed chunk_size, start new chunk
        ...
        if current_tokens + block_tokens <= chunk_size or not current_chunk:
            ...
            # Find blocks from current chunk that fit in overlap
            overlap_blocks = []
            overlap_tokens = 0
